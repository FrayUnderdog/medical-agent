...
- **Trace:** `tool_outputs["knowledge_rag"]["retrieval_trace"]` records offline counts/scores, fallback flags, NHS enablement, result counts, selected sources, and latency. **User-visible `reply` text** is generated by **rule-based answer templates** (`user_reply.py`) so raw retrieved chunks and source paths are **not** shown in the chat bubble; full RAG payloads remain in `tool_outputs` for the developer trace panel.

## Structured intake and patient summary

- **`intake.py`:** deterministic `extract_patient_intake(message, existing_state)` merges name, chief complaint, symptoms, body site, duration, allergies, red-flag hints, suggested department, and a local `risk_level_hint` (not a clinical diagnosis).
- **Session:** each `Session` stores `patient_intake` (full merged dict) server-side across turns.
- **`POST /chat`:** responses include **`patient_summary`** (a UI-shaped subset: name, age, chief complaint, symptoms, duration, body part / side, mechanism, allergies, chronic conditions, suggested department, `risk_level`). `risk_level` combines **guardrails** (emergency/urgent) with intake hints and orchestrator triage.
- **Answer layer:** `format_user_facing_answer` in `user_reply.py` produces short, empathetic replies with red-flag guidance and follow-up questions; **OpenAI/Mock `generate` still runs** for continuity but receives **sanitised** context (no raw RAG blobs in the model payload).
- **Consultation UI:** the left panel reads `patient_summary` from each response. **Developer trace** is a collapsed `<details>` block showing `tool_trace`, `retrieval_trace`, sources, guardrails, and triage JSON—never mixed into the user bubble.
